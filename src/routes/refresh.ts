...
router.get('/auth/refresh', async (request: IRequest, env: Env) => {
	...
	const accessToken = await createAccessToken(cookies.email, env.JWT_SECRET_KEY, env.ENVIRONMENT);
	if (!accessToken) {
	...
	const newRefreshToken = await createRefreshToken(cookies.email, env.JWT_SECRET_KEY, env.ENVIRONMENT);
	if (!newRefreshToken) {
